...
class GameInviteRequestHandlerImpl : GameInviteRequestHandler, KoinComponent {
    ...
    override suspend fun DefaultWebSocketServerSession.handle(session: UserSession, request: WebsocketRequest) {
        ...
        when(request) {
            is GameInviteRequest.Send -> {
                ...
                connections[request.to]!!.webSocketSession.sendSerialized<WebsocketResponse>(GameInviteResponse.Received(session.username))
            }
            is GameInviteRequest.Accept -> {
                ...
                connections[request.from]!!.webSocketSession.sendSerialized<WebsocketResponse>(GameInviteResponse.Started(gameId))
            }
        }
    }
